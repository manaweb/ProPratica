...
if($type == 1){ //cadastro de depoimento
	...
	$msg = "<html><body>";
	$msg .="<p><b><font face= verdana size=2 color=#003366 >Dados enviados em: $d </b></font></p>";
	...
	if ($retorno)
		echo json_encode(array("tipo" => "success", "msg" => "Mensagem enviada com sucesso", "tipoAcao" => 2));
	else
		echo json_encode(array("tipo" => "danger", "msg" => $retorno, "tipoAcao" => 2));
	...
	if(mysql_num_rows($result) == 0){
		mysql_query("insert into tbclientes 
					(nome, empresa, email, telefone, cidade, estado) 
					values 
					('".utf8_decode($nome)."', '".utf8_decode($empresa)."', '".utf8_decode($email)."', '".$telefone."', '".utf8_decode($cidade)."', '$estado')");
	...
	mysql_query("insert into tborcamento (id_cliente, flag_status, data, servicos, mensagem) values ('".$cliente['id']."', 0, '".date('Y-m-d H:i:s')."', '".utf8_decode($checkbox)."', '".utf8_decode($mensagem)."') ");
	echo json_encode(array("tipo" => "success", "msg" => "Orçamento enviado com Sucesso", "tipoAcao" => 3));
}
